...
export default function EditShippingAddressPage() {
  ...
  const handleSubmit = async (payload: Partial<CustomerAddress>) => {
    ...
    try {
      ...
      router.push("/hesap/adresler/teslimat");
    } catch (err: any) {
  ...
  return (
    <AccountShell
      title={t("storefront.account.address.shipping_edit_title", "Teslimat adresini düzenle")}
      subtitle={t("storefront.account.address.shipping_edit_subtitle", "Teslimat bilgilerini güncelleyin.")}
    >
      {loading ? (
        ...
        <>
          ...
          <AddressForm variant="shipping" initial={address} onSubmit={handleSubmit} submitting={saving} />
        </>
      )}
    </AccountShell>
  );
